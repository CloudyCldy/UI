...
const AddHamsterButton = ({ fetchHamsters }) => {
    ...
    const fetchDevicesAndUsers = async () => {
        try {
            const deviceResponse = await axios.get("http://34.224.75.233:3000/devices"); // Fetch devices
            setDevices(deviceResponse.data); // Set the devices in state

            const userResponse = await axios.get("http://34.224.75.233:3000/users"); // Fetch users
    ...
    const handleSave = async () => {
        ...
        try {
            await axios.post("http://34.224.75.233:3000/hamsters", hamsterData); // Send hamster data to the backend
